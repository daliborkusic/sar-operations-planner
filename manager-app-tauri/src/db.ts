...
export async function seedIfEmpty() {
  ...
  for (const mp of mockMissionParticipants) {
    await d.execute(
      'INSERT INTO mission_participants (user_id, mission_id, role, joined_at) VALUES ($1,$2,$3,$4)',
      [mp.userId, mp.missionId, mp.role, mp.joinedAt],
    );
  }
  for (const t of mockTeams) {
    await d.execute(
      'INSERT INTO teams (id, mission_id, name, status, join_code, created_by) VALUES ($1,$2,$3,$4,$5,$6)',
      [t.id, t.missionId, t.name || null, t.status, t.joinCode, t.createdBy],
    );
  }
  for (const tm of mockTeamMembers) {
    await d.execute(
      'INSERT INTO team_members (team_id, user_id, role, joined_at) VALUES ($1,$2,$3,$4)',
      [tm.teamId, tm.userId, tm.role, tm.joinedAt],
    );
  }
  for (const tk of mockTasks) {
    await d.execute(
      'INSERT INTO tasks (id, mission_id, label, search_type, priority, notes, status, assigned_team_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',
      [tk.id, tk.missionId, tk.label, tk.searchType, tk.priority, tk.notes, tk.status, tk.assignedTeamId],
    );
  }
...
interface DbTeamMember { team_id: string; user_id: string; role: string; joined_at: string; }
interface DbTask { id: string; mission_id: string; label: string; search_type: string; priority: string; notes: string; status: string; assigned_team_id: string | null; }
...
export async function loadAll() {
  ...
  const missionParticipants = (await d.select<DbMissionParticipant[]>('SELECT * FROM mission_participants')).map((r) => ({
    userId: r.user_id, missionId: r.mission_id, role: r.role as MissionParticipant['role'], joinedAt: r.joined_at,
  }));
  const teams = (await d.select<DbTeam[]>('SELECT * FROM teams')).map((r) => ({
    id: r.id, missionId: r.mission_id, ...(r.name ? { name: r.name } : {}),
  ...
  const teamMembers = (await d.select<DbTeamMember[]>('SELECT * FROM team_members')).map((r) => ({
    teamId: r.team_id, userId: r.user_id, role: r.role as TeamMember['role'], joinedAt: r.joined_at,
  }));
  const tasks = (await d.select<DbTask[]>('SELECT * FROM tasks')).map((r) => ({
    id: r.id, missionId: r.mission_id, label: r.label, searchType: r.search_type as Task['searchType'],
    priority: r.priority as Task['priority'], notes: r.notes, status: r.status as Task['status'],
  ...
  return { missions, users, missionParticipants, teams, teamMembers, tasks, controllers };
}
